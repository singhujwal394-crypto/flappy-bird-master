...
score = {
    current: 0,
    best: parseInt(localStorage.getItem('ujwal_flappy_best')) || 0,
    ...
    saveBest: function() {
        if (this.current > this.best) {
            ...
            localStorage.setItem('ujwal_flappy_best', this.best)
        }
    },
    //display the score
    render: function() {
        if (gameState.current == gameState.play ||
            ...
            if (gameState.current == gameState.gameOver) {
                ...
                ctx.lineWidth = 3
                ctx.font = "bold 15px 'Carter One', cursive, sans-serif"
                ctx.textAlign = "center"
                ctx.strokeText("BEST: " + this.best, cvs.width / 2, 260)
                ctx.fillText("BEST: " + this.best, cvs.width / 2, 260)
                ctx.restore()
            }
        }
    }
...
document.body.addEventListener('keydown', (e) => {
    if (e.code === 'Space' || e.code === 'ArrowUp' || e.code === 'KeyW' || e.keyCode === 32) {
        e.preventDefault()
        handleAction()
    }
})